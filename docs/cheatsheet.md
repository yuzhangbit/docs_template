...
The beginning and ending symbols (`\(` and `\)` or `$`) are the delimiters for the inline TeX markup.

The tex math block can be called by `$$` and `\[, \]`
```tex
$$-b \pm \sqrt{b^2 - 4ac} \over 2a$$
```
$$-b \pm \sqrt{b^2 - 4ac} \over 2a$$
...
usage: `!!! warning` followed by a new line, then start the content paragraph as below.
```markdown
!!! warning
